...
#define GTK_AXISFACE_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_AXISFACE_KNOB, GtkAxisFaceKnobPrivate))

G_DEFINE_TYPE(GtkAxisFaceKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkAxisFaceKnobPrivate GtkAxisFaceKnobPrivate;

struct _GtkAxisFaceKnobPrivate
{
    int knob_x;
    int knob_y;
    int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
    /** check resize **/
    int grow;
    GtkAxisFaceKnobPrivate *priv = GTK_AXISFACE_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model != 3) {
        ...
        GtkAxisFaceKnob *knob = GTK_AXISFACE_KNOB(widget);
...
{
    g_assert(GTK_IS_AXISFACE_KNOB(widget));
    GtkAxisFaceKnob *knob = GTK_AXISFACE_KNOB(widget);
    GtkAxisFaceKnobPrivate *priv = GTK_AXISFACE_KNOB_GET_PRIVATE(knob);
...
{
    GtkAxisFaceKnob *knob = GTK_AXISFACE_KNOB(object);
...
/****************************************************************
 ** init the GtkAxisFaceKnobClass
 */

static void gtk_knob_class_init (GtkAxisFaceKnobClass *klass)
{
...
//--------- connect the events with funktions
    ...
    g_type_class_add_private(obj_class, sizeof (GtkAxisFaceKnobPrivate));
}
...
static void gtk_knob_init (GtkAxisFaceKnob *knob)
{
...
{
    GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_AXISFACE_KNOB, NULL ));
